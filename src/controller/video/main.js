...
window.setPitchSpeed = function(pitch, playbackRate) {
  _previousPitch = pitch;
  getJungle().setPitchOffset(pitch); // semitones
  _previousPlaybackRate = playbackRate;
  outputNodeMap.forEach((_nodeData, video) => {
...
window.timeoutFn = () => {
  if (++window.speedIndex >= window.speedData.length) {
    window.video.pause();
  } else {
    window.video.currentTime=window.speedData[window.speedIndex][1];
    window.setPitchSpeed(window.pitch,window.speedData[window.speedIndex][2]);
